...
# ---------------------------------------------------------------------------------------
def diversity_lp_model3_create(STRAINS_DATA_ALL,ALLELES_DATA,K1,K2,D_MAX,X_MIN,coeffUsage=1,coeffDev=-1,coeffDevMax=1,coeffDist=-1,LOCI_LIST=[0]):

    # IDEA: penalize the deviation from the average deviation instead of the maximum deviation
    ...
    DIV_M3.set_objective('min',LAMBDA_USAGE*(MAX_STRAINS_USAGE-KNOWN_STRAINS_USAGE)+(DEV_TOTAL*LAMBDA_DEV)+(MAX_DEV*LAMBDA_MAX_DEV)+(TOTAL_HAMMING_D*LAMBDA_DIST))
...
def read_json_3x(JSON_SOL_FILE_NAME,prec):
    ...
    for k,v in RESULTS.items():
        ...
        if k[0:2]=='X_' and val>=prec:            
            ...
            OBJ_VAL[O_key(k)]=val
    return((X_VAL,U_VAL,Z_VAL,E_VAL,DEV_VAL,OBJ_VAL))
...
def json2sol_3x(pref,prec):
    ...
    if os.path.isfile(ILP_JSON):
        (X_VAL,U_VAL,Z_VAL,E_VAL,DEV_VAL,OBJ_VAL) = read_json_3x(ILP_JSON,prec)
        for DICT in  (X_VAL,U_VAL,Z_VAL,E_VAL,DEV_VAL,OBJ_VAL):
...
def txtsol2summary_3(pref):
    ...
    OUTPUT.write('\n')
    OUTPUT.write('Objective function: '+str(OUT['KSU']+OUT['DT']+OUT['MD'])+'\n')
...
if __name__ == "__main__":
    ...
    OUTPUT_PREF       = sys.argv[3]
    K1                = int(sys.argv[4])
    K2                = int(sys.argv[5])
    D_MAX             = int(sys.argv[6])
    X_MIN             = float(sys.argv[7])
    LAMBDA_USAGE      = int(sys.argv[8])
    LAMBDA_DEV        = int(sys.argv[9])
    LAMBDA_DEV_MAX    = int(sys.argv[10])
    LAMBDA_DIST       = int(sys.argv[11])
    PREC = 0.00001
    ...
    M3 = diversity_lp_model3_create(STRAINS_DATA,ALLELES_DATA,K1,K2,D_MAX,X_MIN,LAMBDA_USAGE,LAMBDA_DEV,LAMBDA_DEV_MAX,LAMBDA_DIST)
